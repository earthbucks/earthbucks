...
import {
  ...
  Result,
  SignedWorkData,
} from "@earthbucks/ebx-lib";
import { FixedBuf, PrivKey, SigninResponse } from "@earthbucks/ebx-lib";
  ...
  return {
    ...
    mine: {
      getSignedWorkData: async () => {
        ...
        const { signedWorkData: signedWorkDataHex, prevHeader: prevHeaderHex, prevPrevHeader: prevPrevHeaderHex } = res;
        const signedWorkData = SignedWorkData.fromHex(
          signedWorkDataHex,
          domain,
        );
        const prevHeader = prevHeaderHex ? Header.fromHex(prevHeaderHex) : null;
        const prevPrevHeader = prevPrevHeaderHex ? Header.fromHex(prevPrevHeaderHex) : null;
        return { signedWorkData, prevHeader, prevPrevHeader };
      },
