...
// - 1289: largest prime whose cube is an int32
// - 46337: largest prime whose square is an int32 (does not work on my machine - throws error)
// - 9973: largest prime less than 10,000
// - 1627 the next highest prime number after the number 1624, which is the size of a matrix whose square has the same number of operations as the cube of the 1289 matrix
...
export default function Landing() {
  ...
  async function onComputing() {
    ...
    // gpupow matrixCalculationFloat
    {
      ...
      //   let matrix = gpupow.seedToMatrix1289(seed1289);
      //   matrix = gpupow.matrixCalculation1289(matrix);
      ...
      //   let workingBlockId = blake3Hash(Buffer.from("workingBlockId" + i));
      //   gpupow.updateWorkingBlockId(workingBlockId);
      //   let seed9973 = gpupow.tensorSeed9973();
      //   let matrix = gpupow.seedToMatrix9973(seed9973);
      //   matrix = gpupow.matrixSquare9973(matrix);
      //   let reducedBufs = await gpupow.matrixReduce(matrix);
      //   gpupow.reducedBufsHashAsync(reducedBufs).then((matrixHashBuf) => {
      //     console.log(matrixHashBuf.toString("hex"));
      //   });
      ...
      for (let i = 0; i < 100; i++) {
        let workingBlockId = blake3Hash(Buffer.from("workingBlockId" + i));
        gpupow.updateWorkingBlockId(workingBlockId);
        let seed1627 = gpupow.tensorSeed1627();
        let matrix = gpupow.seedToMatrix1627(seed1627);
        matrix = gpupow.matrixSquare1627(matrix);
        let reducedBufs = await gpupow.matrixReduce(matrix);
        gpupow.reducedBufsHashAsync(reducedBufs).then((matrixHashBuf) => {
          console.log(matrixHashBuf.toString("hex"));
        });
