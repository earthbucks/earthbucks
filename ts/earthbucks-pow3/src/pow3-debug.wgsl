...
const M4_BYTES_SIZE: u32 = MATRIX_SIZE_2D * 4; // 65536
...
struct Pow3 {
    ...
    m4_bytes: array<u32, M4_BYTES_SIZE>,
...
fn convert_m4_to_bytes(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    for (var j: u32 = 0; j < MATRIX_SIZE_2D / 256; j++) {
        ...
        let start: u32 = k * 4;
        pow3.m4_bytes[start] = bytes[0];
        pow3.m4_bytes[start + 1] = bytes[1];
        pow3.m4_bytes[start + 2] = bytes[2];
        pow3.m4_bytes[start + 3] = bytes[3];
    }
}

// now that we have the final matrix converted to bytes, we want to hash it.
// however, instead of simply performing a serial hash on the whole thing, we
// want to break it up into 256 different pieces. if the size of the m4 bytes
// is 65536, then that divided by 256 is 256. so we want to perform sha256 on
// 256 slices of 256 bytes (nice how it works out that way with lots of 256).
// so this produces 256 hashes of size 32, which we store in the many_hash_2
// array, of size 256 * 32 = 8192
@compute @workgroup_size(256, 1, 1)
fn create_many_hash_2_from_m4_bytes(@builtin(global_invocation_id) global_id: vec3<u32>) {
    var i: u32 = global_id.x;

    var start: u32 = i * 256;
    var data: array<u32, 256>;
    for (var k: u32 = 0; k < 256; k++) {
        data[k] = pow3.m4_bytes[start + k];
    }

    var hash_result: array<u32, 32>;
    sha256_full_256(data, &hash_result);
...
// instead of computing m4_bytes first, in this case we create "many hash 2" from
// the m4 matrix directly.
@compute @workgroup_size(256, 1, 1)
