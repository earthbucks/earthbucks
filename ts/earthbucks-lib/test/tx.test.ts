...
describe("Tx", () => {
  ...
  test("to/from u8Vec", () => {
    const version = 1;
    const inputs: TxIn[] = [new TxIn(IsoBuf.alloc(32), 0, new Script(), 0)];
  ...
  describe("isCoinbase", () => {
    test("isCoinbase", () => {
      const version = 1;
      const inputs: TxIn[] = [
        new TxIn(IsoBuf.alloc(32), 0xffffffff, new Script(), 0),
      ];
      const outputs: TxOut[] = [new TxOut(BigInt(100), new Script())];
      const lockAbs = BigInt(0);
  ...
  describe("sighash", () => {
    test("hashPrevouts", () => {
      ...
      expect(IsoBuf.from(result).toString("hex")).toEqual(
    ...
    test("sighash", () => {
      const version = 1;
      const inputs: TxIn[] = [
        new TxIn(IsoBuf.alloc(32), 0, Script.fromEmpty(), 0),
      ];
    ...
    describe("sign and verify", () => {
      it("should generate a deterministic signature", () => {
        ...
        expect(IsoBuf.from(signature.toIsoBuf()).toString("hex")).toEqual(
