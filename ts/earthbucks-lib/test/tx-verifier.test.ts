  ...
  describe("pkh", () => {
    beforeEach(() => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        txOutBnMap.add(txOutBn, EbxBuf.alloc(32), i);
      }
  ...
  describe("pkhxr 1h 40m recoverable", () => {
    beforeEach(() => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const pkh = Pkh.fromPubKeyBuf(EbxBuf.from(key.pubKey.toIsoBuf()));
        pkhKeyMap.add(key, pkh.buf);
        const script = Script.fromPkhxr1h40mOutput(EbxBuf.alloc(32), pkh.buf);
        const txOut = new TxOut(BigInt(100), script);
        const txOutBn = new TxOutBn(txOut, 0n);
        txOutBnMap.add(txOutBn, EbxBuf.alloc(32), i);
      }
  ...
  describe("pkhxr 90d 60d recovery", () => {
    beforeEach(() => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const pkh = Pkh.fromPubKeyBuf(EbxBuf.from(key.pubKey.toIsoBuf()));
        pkhKeyMap.add(key, pkh.buf);
        const script = Script.fromPkhxr90d60dOutput(EbxBuf.alloc(32), pkh.buf);
        const txOut = new TxOut(BigInt(100), script);
        const txOutBn = new TxOutBn(txOut, 0n);
        txOutBnMap.add(txOutBn, EbxBuf.alloc(32), i);
      }
