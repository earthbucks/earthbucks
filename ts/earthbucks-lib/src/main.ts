    ...
    case "key":
      {
        ...
        const privateKeyHex = EbxBuf.from(key.privKey.toIsoBuf()).toString(
          "hex",
        );
        const publicKeyHex = EbxBuf.from(key.pubKey.toIsoBuf()).toString("hex");
    ...
    case "pkh":
      {
        ...
        const publicKeyHex = EbxBuf.from(publicKey).toString("hex");
        const pkhHex = EbxBuf.from(pkh.buf).toString("hex");
        console.log(`Private key: ${privateKeyHex}`);
        console.log(`Public key: ${publicKeyHex}`);
