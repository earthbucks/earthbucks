...
mod tests {
    ...
    fn test_pkh_string_fmt() {
        ...
        let pkh =
            Pkh::from_str("ebxpkh31a042833G3ZzV3uEraE8B2Pvea3rKP2QkaQRVZkxmADrm3LEcN4").unwrap();
        assert_eq!(
            pkh.to_str(),
            "ebxpkh31a042833G3ZzV3uEraE8B2Pvea3rKP2QkaQRVZkxmADrm3LEcN4"
        );
    ...
    fn test_pkh_pairs() {
        ...
        for pair in pkh_data.pkh {
            let pub_key = PubKey::from_str(&pair.pub_key).unwrap();

            // Create a new Address instance
            let pkh = Pkh::from_pub_key(pub_key);

            // Check that the pkh matches the expected pkh
            assert_eq!(pkh.to_str(), pair.pkh);
        }
    }
}
