...
impl Tx {
    ...
    pub fn from_str(hex: &str) -> Result<Self, EbxError> {
...
mod tests {
    ...
    #[test]
    fn test_to_string_and_from_str() -> Result<(), String> {
        let input_tx_id = [0; 32];
        let input_tx_index = 0;
        let script = Script::from_str("DOUBLEBLAKE3 BLAKE3 DOUBLEBLAKE3 EQUAL").unwrap();
        let lock_rel = 0;
        ...
        let hex = tx.to_str();
        let tx2 = Tx::from_str(&hex).unwrap();
        assert_eq!(tx.version, tx2.version);
        assert_eq!(tx.inputs.len(), tx2.inputs.len());
        assert_eq!(tx.outputs.len(), tx2.outputs.len());
        assert_eq!(tx.lock_abs, tx2.lock_abs);
    ...
    fn test_from_coinbase() {
        let input_script = Script::from_str("DOUBLEBLAKE3 BLAKE3 DOUBLEBLAKE3 EQUAL").unwrap();
        let output_script = Script::from_str("DOUBLEBLAKE3 BLAKE3 DOUBLEBLAKE3 EQUAL").unwrap();
